...
mimicTrading.controller('emailLogsCtrl', ['$scope', '$state', 'RestSvr', '$rootScope', 'appSvr', function ($scope, $state, RestSvr, $rootScope, appSvr) {
   ...
   */
		...
		TableAjax.init({
			...
			columns: [{ "data": "id", "orderable": false }, { "data": "email" }, { "data": "type" }, { "data": "timestamp" }, { "data": "action", "orderable": false }]
		});
	});
}]);
'use strict';

mimicTrading.controller('emailLogsViewCtrl', ['$scope', '$state', 'RestSvr', 'appSvr', '$rootScope', 'emailLog', function ($scope, $state, RestSvr, appSvr, $rootScope, emailLog) {
	$scope.$on('$viewContentLoaded', function () {
		/**
   * Initialize the jquery components when view contents loaded properly
   */
	...
	$scope.open = function (action) {
		...
		modalInstance.result.then(function (selectedItem) {
			if (action === 'markAsRead') {
				...
				notificationSvr.deleteNotification(selectedItem).then(function () {
					return $rootScope.manageCount();
				}).then(function (response) {
					return $state.reload();
				}).catch(handleCatch);
			}
...
mimicTrading.config(['$stateProvider', function ($stateProvider) {
	...
	$stateProvider.state('emailLogs', {
		...
		authenticate: true,
		resolve: {
			emailLog: ['$stateParams', 'emailLogsSvr', function ($stateParams, emailLogsSvr) {
				return emailLogsSvr.getLogsList($stateParams.id);
			}]
		}
	});
}]);
...
mimicTrading.factory('cmsSvr', ['RestSvr', function (RestSvr) {
    return {
        ...
        getCmsTypes: function getCmsTypes() {
            return [{key:'about',value:'About'},{key:'staffing',value:'Services --- Staffing'},{key:'managed_services',value:'Services --- Managed Services'},{key:'resources',value:'Resources'},{key:'industries',value:'Industries'},{key:'contact',value:'Contact'}];
        }
    };
}]);
'use strict';
...
mimicTrading.factory('emailLogsSvr', ['RestSvr', function (RestSvr) {
    return {
        getLogsList: function getLogsList(id) {
            return RestSvr.post('email/logs/view/' + id + '/');
        }
    };
}]);
'use strict';
